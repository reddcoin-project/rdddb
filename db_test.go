...
func testNewestShaEmpty(t *testing.T, db rdddb.Db) {
...
func TestEmptyDB(t *testing.T) {
	for _, dbType := range rdddb.SupportedDBs() {
...
func TestAddDuplicateDriver(t *testing.T) {
	...
	// existing one.
	bogusCreateDB := func(args ...interface{}) (rdddb.Db, error) {
...
func TestCreateOpenFail(t *testing.T) {
	...
	// error.
	_, err := rdddb.CreateDB(dbType, "createfailtest")
	if err != openError {
	...
	// error.
	_, err = rdddb.OpenDB(dbType, "openfailtest")
	if err != openError {
		t.Errorf("TestCreateOpenFail: expected error not received - "+
			"got: %v, want %v", err, openError)
		return
...
func TestCreateOpenUnsupported(t *testing.T) {
	...
	// expected error.
	dbType := "unsupported"
	_, err := rdddb.CreateDB(dbType, "unsupportedcreatetest")
	if err != rdddb.ErrDbUnknownType {
		t.Errorf("TestCreateOpenUnsupported: expected error not "+
			"received - got: %v, want %v", err, rdddb.ErrDbUnknownType)
		return
	}

	// Ensure opening a database with the new type fails with the expected
	// error.
	_, err = rdddb.OpenDB(dbType, "unsupportedopentest")
	if err != rdddb.ErrDbUnknownType {
		t.Errorf("TestCreateOpenUnsupported: expected error not "+
			"received - got: %v, want %v", err, rdddb.ErrDbUnknownType)
		return
...
// TestInterface performs tests for the various interfaces of rdddb which
